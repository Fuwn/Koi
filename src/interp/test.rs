use {
  super::{value::Value, *},
  crate::{lexer::new as new_lexer, parser::Parser},
  std::fs,
};
...
fn to_string_dict() {
  ...
  assert!(
    ["{foo: true, bar: 'baz'}", "{bar: 'baz', foo: true}"]
...
fn scopes() {
  assert_eq!(
    output(
      "let name = 'ampere' print(name) {let name = 'thomas the dank engine' \
...
fn dicts_are_refs() {
  assert_eq!(
    output(
      "let x = {a: 1, b: 2, c: 3} let y = x y['a'] = 99 y.b=55 print(x.a, \
       x.b, x.c)",
      None
    ),
    "99 55 3\n".to_string()
...
fn if_else_stmt() {
  assert_eq!(
    output("if false {print(1)} else {print(2)}", None),
    "2\n".to_string()
  );
}
...
fn func() {
  assert_eq!(
    output("fn p() {print('ampere')} p()", None),
    "ampere\n".to_string()
  );
}
...
fn func_return() {
  assert_eq!(
    output("fn p(x) {return x^4} print(p(2))", None),
    "16\n".to_string()
  );
}
...
#[test]
#[should_panic]
fn uncaught_break() { output("break", None); }

#[test]
#[should_panic]
fn uncaught_return() { output("return", None); }

#[test]
#[should_panic]
fn uncaught_break_func_call() { output("fn p() {break} p()", None); }
...
fn func_cannot_access_outer_scope() {
  assert_eq!(
    output("fn f() {print(x)} let x = 10 f()", None),
    "nil\n".to_string()
  );
}
...
fn native_upper() {
  assert_eq!(
    output("print('xyzXYZ123'.upper())", None),
    "XYZXYZ123\n".to_string()
  );
}
...
fn native_lower() {
  assert_eq!(
    output("print('xyzXYZ123'.lower())", None),
    "xyzxyz123\n".to_string()
  );
}
...
fn native_split() {
  assert_eq!(
    output("print('u-v-www-x-y'.split('-')[2])", None),
    "www\n".to_string()
  );
}
...
fn native_for_each() {
  assert_eq!(
    output(
      "let i = 0 let x = [1 2 3 4 5 6] x.forEach(fn(_){i+=1}) print(i)",
      None
    ),
    "6\n".to_string()
  );
...
fn native_string_contains() {
  assert_eq!(
    output("print('abcXXXdef'.contains('XXX'))", None),
    "true\n".to_string()
  );
  assert_eq!(
...
fn native_dict_contains() {
  assert_eq!(
    output("print({a:1 b:2 c:3}.contains('b'))", None),
    "true\n".to_string()
  );
}
