use {
  crate::interp::{cmd::OsEnv, value::Value},
...
impl Var {
  pub fn new(val: Value, is_exp: bool) -> Var { Var { val, is_exp } }
}

impl From<Value> for Var {
  fn from(val: Value) -> Self { Var { val, is_exp: false } }
}
...
impl Env {
  pub fn new() -> Env { Env { map: HashMap::new(), parent: None } }

  pub fn new_from(parent: &Rc<RefCell<Env>>) -> Env {
    Env { map: HashMap::new(), parent: Some(Rc::clone(parent)) }
  }
